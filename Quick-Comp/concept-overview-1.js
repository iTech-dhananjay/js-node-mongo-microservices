...
==> line 85  hasn't been executed but variable are hosted (a) in a separate space or memory (Script) and it was also assigned undefined but it has not been initialized
...
/* 
     ...
     => Nested callback functions creating callback hell
...
// Nested callback functions creating callback hell
function downloadFileFromServer(callback) {
    setTimeout(function() {
...
function processFileData(callback) {
    setTimeout(function() {
...
3. Promise chaining is a method in JavaScript for executing multiple asynchronous tasks in sequence by linking promises together. Each promise's `then` method handles the result of the previous promise, allowing for a streamlined approach to asynchronous programming.
...
createOrder(cart)
    ...
    .then(paymentInfo => console.log(paymentInfo)) // Payment Successful
...
function createOrder(cart) {
    return new Promise((resolve, reject) => {
        ...
        resolve('12345'); // Simulating successful order creation
    });
}
...
/*
   => https://www.youtube.com/shorts/m8biTN2fBEs  - Event loop in javascript
    1. Javascript is synchronous single threaded language and it has one call stack [Present in JS Engine] and handling one task at a time.
    ...
    3. 'window' as global object in web-api has access to setTimeout, console, localStorage, location, fetch,Geolocation web-api without window keyword as it's global
...
/*
   ...
   1. Both spread and rest operators were introduced to JavaScript with ES 6, and both operators are represented with triple dots (...).
   2. The spread operator spreads the values in an array or a string across one or more arguments.
   3. Rest operator allows us to pass an indefinite number of arguments to function by accumulating these several values into an array.
   4. The spread operator in JavaScript expands values in arrays and strings into individual elements, whereas the rest operator puts the values of user-specified data into a JavaScript array.

   => The main difference between rest and spread is that the rest operator puts the rest of some specific user-supplied values into a JavaScript array. But the spread syntax expands iterables into individual elements.
*/

//Q1. Use rest operator to enclose the rest of specific user-supplied values into an array: REST OPERATOR
...
//Q2.  Define a function with three parameters: SPREAD OPERATOR
function myBio(firstName, lastName, company) {
    return `${firstName} ${lastName} runs ${company}`;
}

// Use spread to expand an array’s items into individual arguments:
myBio(...["Oluwatobi", "Sofela", "CodeSweetly"]); // The invocation above will return: “Oluwatobi Sofela runs CodeSweetly”
